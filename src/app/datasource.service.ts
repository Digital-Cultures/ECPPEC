...
export class DatasourceService implements OnChanges {
	...
	uniqueElections: any[] = [];
	//CHANGE uniqueElections: Election[] = [];
	...
	getData() {
		...
		this.getElectionsService.getData().subscribe(
			...
			}, err => console.error("elections data error", err), () => this.gotData());
	...
	gotData() {
		console.log('got data', this.electionsMeta.elections);
		...
		//CHANGE		this.dataSource = new MatTableDataSource<Election>(this.electionsMeta.elections);
		this.dataSource = new MatTableDataSource<any>(this.electionsMeta.elections);
